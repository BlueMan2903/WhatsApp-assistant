...
class AIAssistant:
    ...
    def get_response(self, sender_id: str, user_message: str, image_url: str = None) -> list[str]:
        ...
        try:
            ...
            # For improved debugging, log the raw AI output before cleaning it
            logger.info(f"Raw AI Output for {sender_id}: '{ai_content}'")
            ...
            # Use \s* to handle potential accidental spaces generated by the LLM
            save_name_match = re.search(r'\[ACTION:\s*SAVE_NAME:\s*(.*?)\s*\]', ai_content)
            if save_name_match:
            ...
            other_actions = re.findall(r'\[ACTION:\s*(\w+)\s*\]', ai_content)
            for action in other_actions:
                ...
                # --- RE-IMPLEMENTED HANDOFF LOGIC ---
                elif action == "FORWARD_TO_NIKOL":
                    logger.info(f"Handoff triggered for {sender_id}. Preparing and sending email.")
                    try:
                        # Re-fetch user_name in case it was just saved in this turn
                        user_name_for_email = self.session_manager.get_data(sender_id, 'user_name')
                        subject = f"Web Chat Handoff: {user_name_for_email or sender_id}"
                        body = (
                            f"A user requires manual assistance.\n\n"
                            f"User ID: {sender_id}\n"
                            f"Name (if provided): {user_name_for_email or 'N/A'}\n\n"
                            f"--- Full Transcript ---\n"
                            f"{self.session_manager.get_formatted_history(sender_id)}"
                        )
                        send_error_email(subject, body)
                        logger.info(f"Handoff email for {sender_id} sent successfully.")
                    except Exception as e:
                        logger.error(f"CRITICAL: Failed to send handoff email for {sender_id}: {e}", exc_info=True)
